...
namespace daa_1 {
	...
	byte factfind(byte number) {
		for (byte i = 2; i <= number / 4 + 1; i++) if (number % i == 0) return i;
		return 1;
	}
	...
	void rbt_mainy(const byte real_s, bmap& real_m, list& best_l, byte& best_c, list& curr_l, byte curr_c) {
		...
		for (int i = d.first; i > 0; i--) {
			...
			if (end) {
				...
				if (curr_c + 1 < best_c) rbt_mainy(real_s, real_m, best_l, best_c, curr_l, curr_c + 1);
			}
	...
	list rbt(const byte size) {
		...
		if (fact != 2) {
			temp_l.push_back(elem{ byte((fact - 1) / 2), { (fact + 1) / 2, 0 } });
			temp_l.push_back(elem{ byte((fact - 1) / 2), { 0, (fact + 1) / 2 } });
		}
